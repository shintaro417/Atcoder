...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        long[] amod = new long[46];
        for(int i = 0;i < N;i++){
            a[i] = fs.nextInt();
        }

        long[] bmod = new long[46];
        for(int i = 0;i < N;i++){
            b[i] = fs.nextInt();
        }

        long[] cmod = new long[46];
        for(int i = 0;i < N;i++){
            c[i] = fs.nextInt();
        }

        //46で割った余りがどれだけあるのかを管理する
        for(int i = 0;i <N ;i++){
            //配列amodには配列a内の数字を46で割った余りの個数が保管されている。
            //例えば
            // 3
            //10 13 93
            //5 27 35
            //55 28 52
            //の時にはamodの1,10,13番目の要素の値に1を足している。
            amod[a[i] % 46]++;
            bmod[b[i] % 46]++;
            cmod[c[i] % 46]++;
        }
        ...
        for(int i = 0;i < 46;i++){
            for(int j = 0;j < 46;j++){
                for(int k = 0;k < 46;k++){
                    //i + j + kの合計値が46の倍数であるときに条件を満たす。
                    if((i + j + k) % 46 == 0){
                        //同じ余りを持つ数字が複数存在する場合もあるので、全パターンを求めるときは書く配列の数字をかけ合わせると良い
                        ans += amod[i] * bmod[j] * cmod[k];
                    }
                }
            }
        }
