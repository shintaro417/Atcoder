...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //1 <= l <= rが条件
        for(int i = 1;i <= N;i++){
            //S[i - 1] = S[i] - Kがいくつあるのか数えあげる(常に1 <= l <= rを満たしているはず)
            //連想配列で管理するときにKey : 累積和 value : カウント数とする
            //S[i - 1]のKey値がすでに存在するか否かを判定する
           if(!map.containsKey((Long) S[i - 1])){
               //存在しない場合は新規に追加する。同時に当該Key値を持つ累積和の値が何個あるのかvalueで管理する。
               map.put(S[i - 1],1);
           }else{
               //すでに存在した場合はカウント値を一つ増やして更新する
               map.replace(S[i - 1],map.get(S[i - 1]) + 1);
           }

           //S[i] - Kの値であるKey値が存在する = S[i -1] とS[i] - Kの値が等しいことなので、そのKeyが持っているカウント数を答えに足しこむ
            //存在市内場合はS[i - 1] とS[i] - Kが等しくないので0を返す
